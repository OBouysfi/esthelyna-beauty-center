...
export default function ModalAttribuerPack({ onClose, onSuccess }: ModalProps) {
  ...
  const searchClients = async (query: string) => {
    if (searchCache.current[query]) {
      setClients(searchCache.current[query]);
      return;
    }
    ...
    try {
      ...
      searchCache.current[query] = data;
      setClients(data);
    } catch (err) {
        ...
        <div className="p-5">
          ...
          {step === 1 && (
            <div>
              ...
              {searchClient.length >= 2 && clients.length === 0 && !searchingClients && (
                <p className="text-xs text-gray-500 text-center py-8">
                  Aucun client trouvé
                </p>
              )}
          ...
          {/* Step 2: Sélection Pack */}
          {step === 2 && (
            <div>
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                <p className="text-xs text-amber-800">
                  <strong>Client sélectionné:</strong> {selectedClient?.prenom} {selectedClient?.nom}
                </p>
              </div>
              ...
              <button
                onClick={() => setStep(1)}
                className="mt-4 w-full px-3 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium text-xs"
              >
                Retour
              </button>
            </div>
          ...
          {step === 3 && (
            <div>
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                ...
                <p className="text-xs text-amber-800">
                  <strong>Pack:</strong> {selectedPack?.nom}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">
                    Prix total du pack (DH) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    value={montantTotal}
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Prix standard: {selectedPack?.prix} DH (Modifiable pour remise/promotion)
                  </p>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">
                    Montant payé aujourd'hui (DH) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    value={montantPaye}
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-blue-900 font-bold text-lg">
                        {(parseFloat(montantTotal) - parseFloat(montantPaye)).toFixed(2)} DH
                      </span>
                    </div>
                    {parseFloat(montantPaye) < parseFloat(montantTotal) && (
                      <p className="text-xs text-blue-600 mt-2">
                        💡 Le client pourra payer le reste via la page Paiements
                      </p>
                    )}
                  </div>
                )}
